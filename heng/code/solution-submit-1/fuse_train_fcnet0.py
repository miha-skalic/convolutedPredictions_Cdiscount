...
#
#    [1] "mixup: Beyond Empirical Risk Minimization" - H. Zhang, M. Cisse, Y. N. Dauphin, and D. Lopez-Paz,  arXiv 2017.
#    https://arxiv.org/pdf/1710.09412.pdf

def mixup_data(features, soft_labels, alpha = 0.2):
    batch_size,dim = features.size()
    batch_size,num_classes = soft_labels.size()

    lamda = np.random.beta(alpha + 1, alpha, (batch_size)).astype(np.float32)
    lamda = Variable(torch.from_numpy(lamda)).cuda()

    index = np.random.permutation(batch_size)
    index = Variable(torch.from_numpy(index)).cuda()
    features1 = torch.index_select(features, 0, index)
    soft_labels1 = torch.index_select(soft_labels, 0, index)

    features = features * lamda.view(-1, 1).expand(batch_size,dim) + \
               features1*( 1 - lamda.view(-1, 1).expand(batch_size,dim))
    soft_labels = soft_labels * lamda.view(-1, 1).expand(batch_size,num_classes) + \
                  soft_labels1 * (1 - lamda.view(-1, 1).expand(batch_size,num_classes))

    return features, soft_labels



def valid_augment(features,label,index):
...
def train_augment(features,label,index):
    ...
    soft_label = torch.FloatTensor(CDISCOUNT_NUM_CLASSES).zero_()
    soft_label[label] = 1

    return tensor,label, soft_label, index
...
def run_train():
    ...
    initial_checkpoint = \
        '/root/share/project/kaggle/cdiscount/results/fcnet0-dpn-seresnext-soft-02/checkpoint/00080000_model.pth'
        #None
    ...
    while  i<num_iters:  # loop over the dataset multiple times
        ...
        net.train()
        optimizer.zero_grad()
        for features, labels, soft_labels,  indices in train_loader:
            ...
            # mixup augmentation
            #features, soft_label = mixup_data(features, soft_labels, 0.2)
            features, soft_label = mixup_data(features, soft_labels, 0.3)
            ...
            loss = SoftCrossEntroyLoss()(logits, soft_labels)
            #loss = F.cross_entropy(logits, labels)
