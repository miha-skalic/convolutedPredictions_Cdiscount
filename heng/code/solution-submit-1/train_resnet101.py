...
from net.model.cdiscount.resnet101  import image_to_tensor_transform, ResNet101 as Net
...
def run_training():
    ...
    out_dir  = '/root/share/project/kaggle/cdiscount/results/resnet-101-50b2' # s_xx1'
    initial_checkpoint = \
       '/root/share/project/kaggle/cdiscount/results/resnet-101-50b1/checkpoint/00272000_model.pth'
       #'/root/share/project/kaggle/cdiscount/results/resnet-101-50/checkpoint/00003000_model.pth'

    pretrained_file = \
        None #'/root/share/project/kaggle/cdiscount/model/resnet101-160-08b/checkpoint/00283000_model.pth'
        # None #'/home/ck/project/data/pretrain/SE-ResNet-50.convert.pth'
    skip = []  #'linear.', 'fc.']  #'layer4.', 'fc.']
    ...
    if initial_checkpoint is not None:
        ...
        net.load_state_dict(torch.load(pretrained_file, map_location=lambda storage, loc: storage))
    ...
    ## optimiser ----------------------------------
    iter_accum  = 2
    batch_size  = 768//iter_accum #256//288 #512 #2*288//iter_accum  640
    ...
    valid_dataset = CDiscountImageDataset(
                                    'valid_id_v1_5000', 'train',  mode='train',
    ...
    if 0: ##<debug>
        for n in iter(range(len(train_dataset))):
            m=0  #n
            image, label, index = train_dataset[m]
