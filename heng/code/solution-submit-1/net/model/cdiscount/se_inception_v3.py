...
def run_check_net_cdiscount():
    ...
    checkpoint = '/root/share/project/kaggle/cdiscount/model/se_inception_v3/checkpoint/00051000_model.pth'
    net = SEInception3(in_shape=(3,180,180), num_classes=5270)
    ...
    for i,s in enumerate(product_scores):
        print('%d : %05d,  %0.5f'%(i, np.argmax(s), np.max(s)/255))
...
def run_check_net():
    ...
    net = SEInception3(in_shape=(C,H,W), num_classes=num_classes)
