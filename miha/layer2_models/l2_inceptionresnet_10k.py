...
model_name = "inceptionresnet_v0_wider10k_SELU_cp100_dropout03"
...
def my_reader(filename_queue, feature_names):
    ...
    x_vals = tf.sparse_to_dense(features[feature_names].indices, [1536 * 4],
                                features[feature_names].values)
    x_vals = tf.reshape(x_vals, [4, 1536])
...
read_file = glob('../tf_preprocessing/C_inceptionresnet_v3_cp102_train_2layer/record_*.tfrecords')
read_file_val = glob('../tf_preprocessing/C_inceptionresnet_v3_cp102_val1_2layer/record*.tfrecords')

my_queue = queue_gen(read_files=read_file, batch_size=512, epochs=2000, feat_name="inceptionresnet")
my_val_queue = queue_gen(read_files=read_file_val, batch_size=512, epochs=2000, feat_name="inceptionresnet")

model = Sequential()
model.add(Dense(10000, input_shape=(4, 1536), activation='selu', kernel_initializer='lecun_normal'))
