...
model0 = InceptionResNetV2(include_top=False, weights='imagenet',
...
for clayer in model.layers[3].layers:
    if clayer.name == "block17_1_ac":
        trainable = True
    clayer.trainable = trainable
...
# Train (almost) the whole thing
trainable = False
for clayer in model.layers[3].layers:
    if clayer.name == "max_pooling2d_2":
        trainable = True
    clayer.trainable = trainable
