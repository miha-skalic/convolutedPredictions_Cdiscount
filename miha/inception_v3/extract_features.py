...
# Variables
tf_record_folder = "./C_inceptionv3_v3_cp98_train"
os.makedirs(tf_record_folder, exist_ok=True)
tfrecords_filename = os.path.join(tf_record_folder, 'record_sh{:05d}.tfrecords')

model_xception = load_model("../new_models/models/inceptionv3_v3_part3_098-1.3565067.hdf5")
model_xception = Model(model_xception.input, model_xception.layers[-3].output)
...
while True:
    ...
    for p1, yval in zip(al_xpreds, y_batch):
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'inceptionv3': _floats_feature(np.concatenate(p1)),
        }))
